...
class warehouses:
    ...
    def youShouldPrintYourselfNOW(self):
        for i in range(0,len(self.productslist)):
            print(self.productslist[i].name, self.productslist[i].shops,self.productslist[i].price,self.productslist[i].id)
...
class uselesshouse(warehouses):
    def findcheap(self,inpu):
        ...
        listik.sort()
        for i in range (0, len(self.productslist)):
            if self.productslist[i].name == inpu and self.productslist[i].price == listik[0]:
                print(self.productslist[i].shops)
